...
logging.basicConfig(
    filename=os.path.join(LOG_DIR, f"evaluate_finetuned_{MODEL_NAME}{RUN_SUFFIX}.log"),
    filemode="w",
    level=logging.INFO,
...
logging.info(f"Adapter: {MODEL_ID}")
...
#get predictions with finetuned model
...
def load_model():
    ...
    logging.info(f"Loading adapter: {MODEL_ID}")
...
def main():
    ...
    #calculate precision, recall and f1 with all three evaluation techniques
